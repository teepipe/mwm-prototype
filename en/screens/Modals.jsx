...
function LikeReasonSheet({job, onClose, onSubmit}) {
  ...
  const options = [
    { key: 'wage', label: 'Wage', sub: 'Hourly/monthly is great', icon: ReasonIcons.wage },
    { key: 'location', label: 'Location', sub: 'Easy to commute', icon: ReasonIcons.location },
    { key: 'shift', label: 'Shift', sub: 'Hours work for me', icon: ReasonIcons.shift },
    { key: 'beginner', label: 'Beginner OK', sub: 'Easy to start', icon: ReasonIcons.beginner },
  ];
  return (
    <>
      ...
      <div className="wm-sheet">
        ...
        <div className="wm-sheet-title">What did you like about it?</div>
        <div className="wm-sheet-sub">AI learns and prioritizes similar jobs</div>
...
function FeedbackSheet({job, onClose, onSubmit, firstTime=false}) {
  ...
  const isOn = (key) => selected.includes(key);
  const options = [
    { key: 'far',     label: 'Too far',           icon: ReasonIcons.far },
    { key: 'low',     label: 'Wage too low',      icon: ReasonIcons.low },
    { key: 'type',    label: 'Wrong job type',    icon: ReasonIcons.type },
    { key: 'shift',   label: 'Shift mismatch',    icon: ReasonIcons.shiftX },
    { key: 'nodorm',  label: 'No dorm',           icon: ReasonIcons.nodorm },
    { key: 'other',   label: 'Other',             icon: ReasonIcons.other },
  ];
  return (
    <>
      <div className="wm-sheet-backdrop" onClick={onClose}/>
      <div className="wm-sheet">
        ...
        <div className="wm-sheet-title">
          {firstTime ? 'Tell us a bit more' : "What didn't fit?"}
        </div>
        <div className="wm-sheet-sub">
          {firstTime
            ? "We'll only ask once. We'll use this to improve recommendations."
            : "We'll show fewer jobs like this from now on"}
        </div>
