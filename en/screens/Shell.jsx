...
function LineLiffHeader({title='Mico Work Match', onClose}) {
  return (
    <div className="wm-liff-header">
      <button className="wm-liff-close" onClick={onClose} aria-label="Close">
      ...
      <button className="wm-liff-more" aria-label="Menu">
    ...
    <div className="wm-richbar">
      <div className="wm-richbar-top">
        ...
        <div className="wm-richbar-icons">
          <button className="wm-richbar-iconbtn" aria-label="Notifications">
            <span className="material-symbols-rounded" style={{fontSize:16}}>notifications</span>
            <span className="badge">3</span>
          </button>
          <button className="wm-richbar-iconbtn" aria-label="Filter">
      ...
      <div className="wm-richbar-greet">
        ...
        <span className="wm-richbar-greet-name">{user?.name || 'Sato'}</span>
      </div>

      <div className="wm-richbar-stats">
        <div className="wm-richbar-stat">
          <div className="wm-richbar-stat-label">Today's left</div>
          <div className="wm-richbar-stat-val">{remaining}<span className="unit">/{total}</span></div>
        </div>
        <div className="wm-richbar-stat">
          <div className="wm-richbar-stat-label">Likes</div>
          <div className="wm-richbar-stat-val">{liked}<span className="unit"></span></div>
        </div>
        <div className="wm-richbar-stat">
          <div className="wm-richbar-stat-label">Interviews</div>
          <div className="wm-richbar-stat-val">{interviews}<span className="unit"></span></div>
        </div>
      </div>
  ...
  return (
    <div className="wm-topbar">
      ...
      {right !== null ? right : (counter != null && (
        <div className="wm-pill-counter" title="Today's swipes left">
...
function WMTabBar({current, onNav}) {
  ...
  return (
    <div className="wm-tabbar">
      {tab('daily', 'Top 5', 'auto_awesome')}
      {tab('swipe', 'Swipe', 'style')}
      {tab('history', 'Likes', 'favorite')}
      {tab('me', 'My Page', 'person')}
    </div>
  );
}
